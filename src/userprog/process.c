...
//Changes by Sayem for wait-simple, twice, multiple 
/*
Checking if the child process exists before proceeding.
Checking if the child process has already been waited on.
Using a semaphore to wait for the child process to exit, instead of a busy loop.
Removing the child process from the list of child processes after it has exited.
*/

/* Changes by Sayem: A system call handler that waits for a child process to finish and returns its exit status. Uses the wait_sema
 semaphore in the child process's struct child_process
 to wait.
*/
int process_wait(tid_t child_tid UNUSED)
{
  ...
  if (cp->wait)
  {
    // printf("wait has been called on this child proc\n");
    return ERROR;
  }
  cp->wait = true;
  ...
  //     timer_sleep(1000);
  //   }
  sema_down(&cp->wait_sema); // Changes
...
process_exit (void)
{
  ...
  // Changes: Close all files opened by the current process
  //// Start at the beginning of the current thread's list of open files
  ...
  // Changes by Sayem
  // When the process is done executing the file to allow it to be writable again. 
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  *eip = (void (*) (void)) ehdr.e_entry;
  ...
  //Changes for rox-simple
  // Block write access to the running executable file by maintaining a ref to the executable file
  if (success)
  {
    file_deny_write(file);
