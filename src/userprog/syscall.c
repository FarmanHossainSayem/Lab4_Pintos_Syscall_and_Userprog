...
//Changes Sayem
 case SYS_READ:
    ...
    f->eax = s_read(fd_read, buf, bufsize);
    break;
 case SYS_WRITE:
    get_args(f->esp, (void **)&s_args, 3);
    int fd_rw = (int)(s_args[0]);
    bufsize = (unsigned int)(s_args[2]);
    check_buffer_ptr((char *)(s_args[1]), bufsize);
    buf = (char *)user_to_kernel_ptr((const void *)s_args[1]);
 ...
 case SYS_WAIT:
    get_args(f->esp, (void **)&s_args, 1);
    tid_t tid = (tid_t)(s_args[0]); // tid means child process
    f->eax = s_wait(tid);
    break;
...
//changes Sayem: Wait for a specific child process to finish and returns the status code (CASE SYS_WAIT)
int s_wait(tid_t tid)
{
...
*/
//changes Sayem: exit the current thread and set its status to the given status parameter. 
void s_exit(int status)
{
  ...
  {
    curthread->cp->status = status; // set up child status
  }
...
//changes: added seek function
void s_seek(int fd, unsigned position)
{
  lock_acquire(&file_lock); //Nasif Changes
  struct file *fp = get_file(fd);
  if (fp == NULL){
    lock_release(&file_lock);
    return;
  } else {
    file_seek(fp, position); //call seek if file is foind
  }
...
*/
// Changes for bad-read and bad-write Sayem
void check_invalid_ptr_error(const void *vaddr)
{
...
verify_user (const uint8_t *uaddr)
{
  ...
  if (pagedir_get_page(thread_current()->pagedir, uaddr) == NULL) return false; //Checks if the user address is mapped.
  // is it mapped?  How do I check this?
  return true;
}

// Changes for wait-twice Sayem
// Creates a new struct child_process for a child thread and add it to the parent thread's child list.
// 
struct child_process* add_child_process (int pid)
{
  ...
  sema_init(&cp->wait_sema,0); //Changes initializing wait_sema
